      ...
      <div className="section-container relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {pillars.map((pillar, i) => (
            <motion.div key={pillar.title} custom={3 + i} variants={fadeUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'}
              className="glass-card-hover rounded-xl p-7 flex flex-col" data-testid={`squad-card-${i}`}>
              <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-5"
                style={{ background: pillar.color === 'primary' ? 'hsl(216 100% 50% / 0.08)' : 'hsl(259 72% 58% / 0.08)' }}>
