...
export const CTASection = ({ cms }: { cms?: any }) => {
  const ref = useRef(null)
  const isInView = useInView(ref, { once: true, margin: '-80px' })
  const [contactOpen, setContactOpen] = useState(false)

  const label = cms?.label || 'About ACE inovations'
  const hp1 = cms?.headingPart1 || 'Built by engineers,'
  const hhl = cms?.headingHighlight || 'for builders'
  const desc = cms?.description || 'ACE inovations is a modern engineering and growth studio. We combine world-class software development with agentic AI workflows to deliver end-to-end digital solutions that scale.'
  const btnText = cms?.ctaButtonText || 'Start a Project'
  const email = cms?.email || 'hello@aceinovations.dev'

  return (
    <>
      <section id="about" ref={ref} className="relative py-24 lg:py-32 overflow-hidden" data-testid="cta-section">
