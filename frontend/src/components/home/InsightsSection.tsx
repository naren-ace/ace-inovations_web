...
const articles = [
  {
    ...
    readTime: '8 min read', date: 'Jan 2026', color: 'primary',
  ...
  },
  {
    ...
    readTime: '12 min read', date: 'Dec 2025', color: 'accent',
  ...
  },
  {
    ...
    readTime: '6 min read', date: 'Nov 2025', color: 'primary',
  ...
  },
  {
    ...
    readTime: '10 min read', date: 'Oct 2025', color: 'accent',
  ...
  },
  {
    ...
    readTime: '7 min read', date: 'Sep 2025', color: 'primary',
    accentBg: 'linear-gradient(135deg, hsl(216 100% 50% / 0.04), hsl(216 100% 65% / 0.02))',
  },
]
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-12">
          <div>
            <motion.p custom={0} variants={fadeUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'}
              className="text-xs font-medium tracking-[0.2em] uppercase mb-4" style={{ color: 'hsl(var(--primary))' }}>Insights</motion.p>
            <motion.h2 custom={1} variants={fadeUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'}
              className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-foreground leading-tight">From the Lab.</motion.h2>
            <motion.p custom={2} variants={fadeUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'}
              className="mt-3 text-base md:text-lg leading-relaxed max-w-lg" style={{ color: 'hsl(var(--body))' }}>
              Deep dives into engineering, AI, and the strategies driving modern digital product development.
            </motion.p>
          </div>
      ...
      <motion.div custom={3} variants={fadeUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'}>
        <div ref={scrollRef} onScroll={checkScroll}
          ...
          {articles.map((article) => (
            <article key={article.title} className="flex-none w-[340px] sm:w-[360px] snap-start group">
